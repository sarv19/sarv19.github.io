...
const Experience = ({experiences, text, link, anchor, showProject, maxProjects}) => {
    let itemsToDisplay;

    if (maxProjects === null) {
        itemsToDisplay = experiences;
    } else {
        itemsToDisplay = experiences.slice(0, maxProjects);
    }
    return (
        <Element className='experience' name={anchor}>
            {anchor && <div className='section-title'>{anchor}</div>}
            {
                itemsToDisplay && itemsToDisplay.map((exp, index) => (
                    <ExperienceBlock experience={exp} key={index}/>
                ))
            }
            { text &&
                <a
                    href={showProject ? '/projects' : link}
                    target={showProject ? '_self' : '_blank'}
